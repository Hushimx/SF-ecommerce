        ...
        <div class="special-box">
            ...
            <table class="table">
              <tbody>
                ...
                <tr>
                  ...
                  <td width="10%">:</td>
                  <td width="45%">{{$order->currency_sign}}{{ round($order->pay_amount * $order->currency_value , 2) }}
                  </td>
                </tr>
                <tr>
          ...
          <div class="footer-area">
            ...
            <select name="" class="process select order-droplinks {{ strtolower($order->status) }}">
              <option data-val="pending" {{ $order->status == 'pending' ? 'selected' : '' }}
                value="{{ route('admin-order-status',['id1' => $order->id, 'status' => 'pending']) }}">
                {{ __('Pending') }}</option>
              <option data-val="processing" {{ $order->status == 'processing' ? 'selected' : '' }}
                value="{{ route('admin-order-status',['id1' => $order->id, 'status' => 'processing']) }}">
                {{ __('Processing') }}</option>
              <option data-val="completed" {{ $order->status == 'completed' ? 'selected' : '' }}
                value="{{ route('admin-order-status',['id1' => $order->id, 'status' => 'completed']) }}">
                {{ __('Completed') }}</option>
              <option data-val="declined" {{ $order->status == 'declined' ? 'selected' : '' }}
                value="{{ route('admin-order-status',['id1' => $order->id, 'status' => 'declined']) }}">
                {{ __('Declined') }}</option>
            </select>
            <a href="{{ route('admin-order-invoice',$order->id) }}" class="mybtn1"><i class="fas fa-eye"></i>
              {{ __('View Invoice') }}</a>
          </div>
        </div>
      </div>
            ...
            <table class="table">
              <tbody>
                ...
                <tr>
                  ...
                  <th width="10%">:</th>
                  <td width="45%">
                    {{(DB::table('countries')->where('id','=', $order->customer_country)->first()->name_ar)}}</td>
                </tr>
                <tr>
                  <th width="45%">{{ __('City') }}</th>
                  <th width="10%">:</th>
                  @if (is_numeric($order->customer_city))
                  <td width="45%">{{(DB::table('cities')->where('id','=', $order->customer_city)->first()->name_ar)}}
                  </td>
                  @else
            ...
            <table class="table">
              <tbody>
                ...
                <tr>
                  ...
                  <th width="10%">:</th>
                  <td width="45%">{{$order->shipping_email == null ? $order->customer_email : $order->shipping_email}}
                  </td>
                </tr>
                <tr>
                  <th width="45%"><strong>{{ __('Phone') }}:</strong></th>
                  <th width="10%">:</th>
                  <td width="45%">{{$order->shipping_phone == null ? $order->customer_phone : $order->shipping_phone}}
                  </td>
                </tr>
                <tr>
                  ...
                  <th width="10%">:</th>
                  <td width="45%">
                    {{$order->shipping_address == null ? $order->customer_address : $order->shipping_address}}</td>
                </tr>
                <tr>
                  <th width="45%"><strong>{{ __('Country') }}:</strong></th>
                  <th width="10%">:</th>
                  <td width="45%">
                    {{$order->shipping_country == null ? '' : (DB::table('countries')->where('id','=', $order->customer_country)->first()->name_ar)}}</td>
                </tr>
                <tr>
                  <th width="45%"><strong>{{ __('City') }}:</strong></th>
                  <th width="10%">:</th>
                  @if (is_numeric($order->shipping_city))
                  <td width="45%">{{(DB::table('cities')->where('id','=', $order->shipping_city)->first()->name_ar)}}
                  </td>
                  @else
                ...
                <tr>
                  ...
                  <td>
                      ...
                      @for($j=0; $j<=$product['qty']-1; $j++) <?php $ind++; ?> @if(isset($order->wock_serials[$ind]))
                        ...
                        @for($k=0; $k<count($order->wock_serials[$ind]); $k++)
                          ...
                          @if($k<count($order->wock_serials[$ind])-1)
                            ...
                            @endif
                            @else
                            <img width="180px" src="data:image/png;base64, {{ $order->wock_serials[$ind][$k] }}"
                              alt="{{ $langg->lang320 }}  {{$i}}" />
                            @endif
                            @endfor
                    ...
                    <a href="javascript:;" data-toggle="modal" data-target="#confirm-delete"
                      class="btn btn-info product-btn" id="license" style="padding: 5px 12px;"><i class="fa fa-eye"></i>
                      {{ __('View License') }}</a>
                    @endif
                  ...
                  <td>{{$product['size']}}</td>
                  <td><span
                      style="width: 40px; height: 20px; display: block; background: {{$product['color']}};"></span></td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p class="text-center"><span id="key"></span><a href="javascript:;"
            id="license-edit"><br />{{ __('Edit License') }}</a><a href="javascript:;" id="license-cancel"
        ...
        <form method="POST" action="{{route('admin-order-license',$order->id)}}" id="edit-license"
          ...
          <div class="form-group text-center">
            <input type="text" name="license" placeholder="{{ __('Enter New License Key') }}"
              style="width: 40%; border: none;" required=""><input type="submit" name="submit" class="btn btn-primary"
