...
class PaymentController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $paypal_conf['client_id'] = "AaZOHHgQ_XF7JHptar9HvxVQdU5vsniMb7u4eV-0_0XkkSZc_pNE1VGCb3PC2jJe28zBy7niw0MHnRSP";
        $paypal_conf['secret'] = "EBUUPZZs6meGPV64o1Jx8eB5QnCXkfV_JtcDduBC0lX2VLh4fmhjxkf1qfs-FdzJ7WEiVL3VYi1jM4Sz";
    ...
    public function store(Request $request)
    {
        ...
        if ($curr->sign == "S.R") {
            $order['item_amount'] = ($request->total + $request->shipping_cost + $request->tax ) / 3.75;
        } elseif ($curr->sign  == "$") {
            $value2 = ($request->total + $request->shipping_cost + $request->tax);
    ...
    public function notify(Request $request)
    {
        ...
        if ($result->getState() == 'approved') {
            ...
            foreach($cart->items as $key => $prod)
            {
                if(!empty($prod['item']['license']) && !empty($prod['item']['license_qty']))
                {
                    ...
                    $order['customer_email'] = $paypal_data['shipping_email'];
                    $order['customer_name'] = $paypal_data['shipping_name'];
                    $order['customer_phone'] = $paypal_data['shipping_phone'];
                    $order['order_number'] = $order_data['item_number'];
                    $order['shipping'] = $paypal_data['shipping_cost'];
                    $order['pickup_location'] = "";
                    $order['customer_address'] = $paypal_data['shipping_address'];
                    $order['customer_country'] = $paypal_data['country'];
                    $order['customer_city'] = $paypal_data['city'];
                    $order['customer_zip'] = $paypal_data['shipping_zip'];
                    ...
                    $order['shipping_country'] = $paypal_data['country'];
                    $order['shipping_city'] = $paypal_data['city'];
        ...
        if($gs->is_smtp == 1)
        {
            ...
            {
                $prod44 .= '<tr class="all hd"><td class="all hd">'.$prod12["item"]["name"]. '</td><td class="all hd">'.$prod12['qty'].'</td><td class="all hd">'. $this->curr->sign . number_format($prod12["item"]["price"] * $order->currency_value,2,'.','') .'</td> <td class="all hd">'. $this->curr->sign . number_format($prod12["price"] * $order->currency_value,2,'.','') .'</td></tr>';
            }

            $prod44 .= '<tr class="all hd"><td class="all hd">Shipping</td><td class="all hd">1</td><td class="all hd">'. $this->curr->sign . number_format($order->shipping_cost,2,'.','') .'</td> <td class="all hd">'. $this->curr->sign . number_format($order->shipping_cost,2,'.','') .'</td></tr>';

            $prod44 .= '<tr class="all hd"><td class="all hd">Tax</td><td class="all hd">1</td><td class="all hd">'. $this->curr->sign . $order->tax .'</td> <td class="all hd">'. $this->curr->sign . $order->tax .'</td></tr>'.'</br>';
