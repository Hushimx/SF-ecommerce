...
class CheckoutController extends Controller
{
    ...
    public function checkout()
    {
        ...
        $first_order->customer_zip == '';
        $first_order->customer_phone == '';
        $first_order->customer_address == '';
        ...
        if($user) {
            ...
            $first_order = Order::where('user_id', '=', $user->id)->where('dp', '=', 0)->first();
            if($first_order) {
                $country =  Country::where('id','=',$first_order->customer_country)->first();
                if($country) {
                    $cities = City::where('country_id','=',$country->id)->get()->toArray();
                } else {
                    $country =  new Country;
                    $country->name_ar == '';
                    $country->name_en == '';
        ...
        if(Auth::guard('web')->check()) {
            ...
            if($dp == 1) {
                ...
                $grp_shipping_countries = [];  

            } else {

                $shipping_countries = DB::table('cities')
                ...
                $grp_shipping_countries = $shipping_countries->unique('id')->toArray();
                $countries = [];
            ...
            if(!Session::has('coupon_total')) {
                ...
                if (strpos($total, 'S.R') !== false) {
                    $total = floatval(str_replace("S.R", "", $total));
                } elseif ((strpos($total, 'R.S') !== false)) {
                    $total = floatval(str_replace("R.S", "", $total));
                } else {
            ...
            return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways,'digital' => $dp, 'curr' => $curr,'countries' => $countries, 'grp_shipping_countries' => $grp_shipping_countries, 'first_order'=>$first_order, 'cities'=>$cities, 'country'=>$country] );             
            ...
            // If guest checkout is activated then user can go for checkout
           	if($gs->guest_checkout == 1) {
                ...
                if($dp == 1) {
                    ...
                    $shipping_countries = DB::table('countries')->join('shippings', 'countries.id', '=', 'shippings.country_id')->get();
            
                    $countries = $shipping_countries->unique('country_id')->toArray();
                ...
                foreach ($products as $prod) {
                    ...
                    if($prod['item']['type'] != 'Physical') {
                        ...
                        if(!Auth::guard('web')->check()) {
                            ...
                            $ck = 1;
                            return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'checked' => $ck, 'digital' => $dp, 'curr' => $curr, 'first_order'=>$first_order, 'cities'=>$cities, 'country'=>$country]);  
                        }
                    }
                }
                
                return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'digital' => $dp, 'curr' => $curr, 'first_order'=>$first_order, 'cities'=>$cities, 'country'=>$country]);                 
                ...
                return view('front.checkout', ['products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'checked' => $ck, 'digital' => $dp, 'curr' => $curr, 'countries' => $countries, 'grp_shipping_countries' => $grp_shipping_countries, 'first_order'=>$first_order, 'cities'=>$cities, 'country'=>$country]);                 
            }
        }
    ...
    public function cashondelivery(Request $request)
    {
        if($request->pass_check) {
            ...
            if(count($users) == 0) {
                if ($request->personal_pass == $request->personal_confirm){
                    ...
                    $token = md5(time().$request->shipping_name.$request->shipping_email);
        ...
        $order['pay_amount'] = round($request->total + $request->shipping_cost , 2);
  ...
  if($gs->is_smtp == 1)
    ...
    public function getshippingmethod()
    {
        ...
        $city_id = $_GET['cityId'];
        $country_id = $_GET['countryId'];
        $fnd = Shipping::where('country_id','=',$city_id)->get()->count();
        if($fnd < 1)
        {
            ...
            $data['cities'] = City::where('country_id','=',$country_id)->get();
            $data['methods'] = Shipping::where('country_id','=',$city_id)->get();
            if($curr->sign == "$"){
                $data['methods'] =DB::table("shippings")->select(DB::raw('(price/3.75) AS price'), 'title', 'subtitle', 'id', 'image')->where('country_id','=',$country_id)->get();
            }
            return response()->json($data); 
        }  
