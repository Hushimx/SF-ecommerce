...
class CartController extends Controller
{
    ...
    public function cart()
    {
        ...
        $this->code_image();
        if (!Session::has('cart')) {
            return view('front.cart', compact('curr'));
        }
   ...
   public function addcart($id)
    ...
    public function coupon()
    {
        ...
        if($fnd < 1) {
            ...
            if (strpos($_GET['total'], 'S.R') !== false) {
                $total = floatval(str_replace("S.R", "", $_GET['total']));
            } elseif ((strpos($_GET['total'], 'R.S') !== false)) {
                $total = floatval(str_replace("R.S", "", $_GET['total']));
            } else {
                $total = floatval(str_replace("$", "", $_GET['total']));
            }
