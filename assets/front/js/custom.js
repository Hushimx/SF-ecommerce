  ...
  $(document).ready(function () {
    ...
    $(document).on("click", ".quick-view", function () {
      ...
      $(".quick-view-modal").load(
        $(this).data("href"),
        function (response, status, xhr) {
          if (status == "success") $(".submit-loader").hide();
    ...
    $(document).on("click", ".add-to-cart", function () {
      $.get($(this).data("href"), function (data) {
        if (data == "digital") {
          ...
          $("#modal-content-cart").load(mainurl + "/carts/view");
    ...
    $(document).on("click", ".cart-remove", function () {
      ...
      $.get($(this).data("href"), function (data) {
        ...
        $("#grandtotalcart").val(data[3]);
        if (pos == 0) {
          $(".discount").html(sign + 0);
        } else {
          $(".discount").html(0 + sign);
        } 
                
 
        if (data == 0) {
          $("#cart-count").html(data);
          if (mainurl + "/checkout" == pathname || mainurl + "/carts" == pathname) {
            setTimeout(function() {
        ...
        if (mainurl + "/checkout" == pathname && data != 0 ) {
          
            var shippingValue = $("#shipping-cost").val();
            var totalValue = data[3].match(regex).map(function (v) {
              return parseFloat(v);
            });
            ...
            ftotal = parseFloat(shippingValue) + totalValue[0] + taxValue[0];

            $(".cart-quantity").html(data[1]);
            $(".coupon-total").val(data[0]);
            $("#total-cost").html(data[3]);
            //$("#final-cost").html(data[3]);
            $("#grandtotalcart").val(data[0]);
            $(".dpercent").html("");

            if (pos == 0) {
              $("#discount").html(sign + 0);
              $("#final-cost").html(sign + ftotal);
            } else {
              $("#discount").html(0 + sign);
              $("#final-cost").html(ftotal + sign);
            }          
        }
      });
    });
    ...
    /*$('.ship-diff-addres-area ul li').on('click', function() {
    ...
    /***** start city change ****/
    $(".ship-diff-addres-area ul li").on("click", function () {
      var countryId = $(this).find('img').data('value');
